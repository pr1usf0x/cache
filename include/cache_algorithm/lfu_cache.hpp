...
private:
    ...
    Tp MoveOldNode(const Key& key, typename std::unordered_map<Key, ElInfo>::iterator hash_it) {
        ...
        size_t next_freq_num = (next_list_it == frequencies_.end() ? 0 : next_list_it->freq_num);
    ...
    Tp MoveNode(const Key& key, ListIt& list_it, ListIt& next_list_it, ElemIt& elem_it) {
    ...
    Tp CreateAndMove(const Key& key, ListIt& list_it, ListIt& next_list_it, ElemIt& elem_it) {
        Tp elem_val = elem_it->val;
        ++(elem_it->counter);

        auto freq_it = frequencies_.insert(next_list_it, FreqInfo{elem_it->counter});
    ...
    Tp AddNewElement(const Key& key, std::function<Tp(const Key& key)> slow_get_page) {
        Tp elem_val = slow_get_page(key);
        ListIt first_list = frequencies_.begin();

        if (first_list != frequencies_.end() && first_list->freq_num == kMinCounter) {
            CacheList& cache_list = first_list->cache_list;
            auto new_it = cache_list.emplace(cache_list.end(), CacheNode{key, elem_val, kMinCounter});
            data_base_[key] = {new_it, first_list};
        } else {
            CacheList new_list;
            auto new_elem_it = new_list.emplace(new_list.end(), CacheNode{key, elem_val, kMinCounter});
            auto freq_it = frequencies_.insert(frequencies_.begin(), FreqInfo{kMinCounter});
    ...
    void DisplayListOfLists(std::ostream& out) const {
        ...
        out << "Total Capacity: " << cache_cap_ << " | Total Elements: " << cache_sz_
            << '\n';
    ...
    void DisplayList(std::ostream& out, const CacheList& list) const {
        ...
        for (auto it = list.begin(); it != list.end(); ++it) {
            out << "\t\t[" << num << ']' << ": Key = " << it->key << ", counter = " << it->counter << '\n';
            ++num;
        }
    }
